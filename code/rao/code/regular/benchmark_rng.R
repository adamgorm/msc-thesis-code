...
n_tib <- read.table(paste0(base_dir, "sim_args_rng.txt"), header = TRUE)
sim_dir <- paste0(base_dir, "/sim_data/")
time_dir <- paste0(base_dir, "/time_mins_rng/")
...
{
    smart_implementation <- arg_tib$implementation == "smart"
    ...
    if (arg_tib$implementation == "base") {
        t1 <- Sys.time()
        stan_funs$mu_eta_reg_base_rng(
                      sim_dat$y_obs_vec,
                      sim_dat$t,
                      t_pred,
                      sim_dat$n_obs,
        ...
        if (arg_tib$implementation == "smart") {
            testfun <- stan_funs$mu_eta_reg_smart_rng
        } else if (arg_tib$implementation == "iter") {
            testfun <- stan_funs$mu_eta_reg_iter_rng
        } else if (arg_tib$implementation == "smart_iter") {
            testfun <- stan_funs$mu_eta_reg_smart_iter_rng
        }
        t1 <- Sys.time()
        testfun(
            sim_dat$y_obs_vec,
...
complete_tib <- do.call(rbind,
                        lapply(c("smart_iter", "smart", "base"),
...
for (i in 1:length(split_complete_tib)) {
    cat(sprintf("%d / %d : ", i, length(split_complete_tib)))
