...
n_tib <- read.table(paste0(base_dir, "sim_args_full_fit_base.txt"), header = TRUE)
sim_dir <- paste0(base_dir, "/sim_data/")
time_dir <- paste0(base_dir, "/time_mins_full_fit_base/")
time_no_gq_dir <- paste0(time_dir, "/no_gq/")
lapply(c(sim_dir, time_dir, time_no_gq_dir),
       \(x) dir.create(x, showWarnings = FALSE))

sim_mod <- cmdstanr::cmdstan_model("../sim.stan")
fit_mods <- list(
    no_gq = cmdstanr::cmdstan_model("base_no_gq.stan")
)
