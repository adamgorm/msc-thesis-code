/*
    ...
    Change of notation in thesis, compared to parts of the implementation.
    Left hand side = thesis notation.
    Right hand side = some of the implementation notation.
...
functions {
                         ...
                         real sigma, matrix one_mat_n_a)
  {
    ...
    tuple(vector[J_pred], matrix[J_pred, J_pred],
          ...
          matrix[N_pred_except_1_group,
                 N_pred_except_1_group]) mu_eta_mean_cov_post_chol =
      mu_eta_irreg_smart_post(y_a, y_a_vec, y_b_vec, y_ab_vec,
                              t_a, t_b, t_pred,
                              n_a, n_b,
                              J_pred, J_a, J_b,
                              t_b_is,
                              ...
                              magnitude_eta, length_scale_eta,
                              sigma, one_mat_n_a);
    return mu_eta_post_chol_rng(J_pred, n, mu_eta_mean_cov_post_chol);
  }
  ...
  tuple(vector, matrix, vector, matrix)
  mu_eta_irreg_smart_post(matrix y_a, vector y_a_vec, vector y_b_vec, vector y_ab_vec,
                          array[] real t_a, array[] real t_b, array[] real t_pred,
                          int n_a, int n_b, int J_pred,
                          int J_a, array[] int J_b, array[] int t_b_is,
                          real magnitude_mu, real length_scale_mu,
  ...
  */
  ...
  tuple(vector, matrix, vector, matrix, matrix, vector, matrix, matrix)
                       ...
                       real sigma)
  {
    ...
    matrix[Npred_except_1_group, Npred_except_1_group] cov_q_eta =
      cov_eta_cond_o[:Npred_except_1_group, :Npred_except_1_group] -
      cov_eta_c_cond_o[:Npred_except_1_group, ]
      * (cov_c_cond_o \ cov_eta_c_cond_o[:Npred_except_1_group, ]');

    vector[Npred] mean_eta_cond_o = cov_eta_o * (cov_o \ yo);
    vector[Nc] mean_c_cond_o = cov_c_o * (cov_o \ yo);
    matrix[Npred_except_1_group, Nc] p_factor_eta =
      (cov_eta_c_cond_o / cov_c_cond_o)[:Npred_except_1_group, ];
    ...
    matrix[Jpred, Jpred] cov_q_mu =
      cov_mu_cond_o - cov_mu_c_cond_o * (cov_c_cond_o \ cov_mu_c_cond_o');

    vector[Jpred] mean_mu_cond_o = K_mu_pred_o * (cov_o \ yo);
    matrix[Jpred, Nc] p_factor_mu = cov_mu_c_cond_o / cov_c_cond_o;
    ...
    return (mean_c_cond_o,
            ...
            p_factor_mu,
            cov_q_mu,
            mean_eta_cond_o[:Npred_except_1_group],
            p_factor_eta,
            cov_q_eta);
  }
