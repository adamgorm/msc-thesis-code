...
tib_flat_loglik <- filter(res_tib, n_obs == 100, n_group == 100, type == "loglik")
...
irreg_nanb <- filter(res_tib, J == 100, n_a + n_b == 50, is.na(J_pred) | J_pred == 100) |>
    ...
    facet_wrap(~ type) +
    time_lab +
    ggtitle("n_a + n_b = 50, J = 100, J_p = 100") +
    lty_manual_irreg +
    col_manual_irreg +
    theme(legend.position = "none") +
    theme(plot.title = element_text(size=11))
...
tib_flat_loglik_irreg <- filter(res_tib, n_b == 10, n_a == 90, J == 50, type == "loglik")
...
filter(res_tib, J == 100, n_a + n_b == 100, is.na(J_pred) | J_pred == 100) |>
    ...
    facet_wrap(~ type) +
    time_lab +
    ggtitle("n_a + n_b = 100, J = 100, J_p = 100")
...
(reg_n + guides(linetype = guide_legend(override.aes = list(linewidth = 0.5)))) /
    (reg_J + guides(linetype = guide_legend(override.aes = list(linewidth = 0.5)))) /
    (reg_Jp + guides(linetype = guide_legend(override.aes = list(linewidth = 0.5)))) +
    plot_layout(guides = "collect") &
    theme(legend.position = "bottom",
...
save_fig_full_page("regular_nonlog")
